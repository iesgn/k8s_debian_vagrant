...
En el directorio de trabajo, creamos el directorio CA/ en el que ubicaremos todos los certificados:
...
cfssl gencert \
  ...
  -ca-key=ca-key.pem \
  -config=ca-config.json \
  -hostname=10.0.0.2,10.0.0.3,10.0.0.4,127.0.0.1,${KUBERNETES_HOSTNAMES} \
  -profile=kubernetes \
...
```
for instance in `seq 1 3`; do
	scp -i .vagrant/machines/node${instance}/libvirt/private_key CA/ca.pem \
	CA/node${instance}-key.pem CA/node${instance}.pem vagrant@node${instance}:~/
done
```
...
```
for instance in `seq 1 3`; do
	scp -i .vagrant/machines/controller${instance}/libvirt/private_key CA/ca.pem \
	CA/ca-key.pem CA/kubernetes-key.pem CA/kubernetes.pem \
    CA/service-account-key.pem CA/service-account.pem vagrant@controller${instance}:~/
done
```
