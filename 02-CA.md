...
cfssl gencert \
  ...
  -ca-key=ca-key.pem \
  -config=ca-config.json \
  -hostname=10.0.10.1,192.168.122.1,10.0.10.2,10.0.10.3,10.0.10.4,127.0.0.1,${FLOATING_IP},${KUBERNETES_HOSTNAMES} \
  -profile=kubernetes \
...
for instance in node{1..3}; do
	scp Config/ca.pem Config/${instance}-key.pem Config/${instance}.pem root@${instance}:~/
done
```
...
scp Config/ca.pem Config/kube-proxy.pem Config/kube-proxu-key.pem \
Config/ca-key.pem Config/kubernetes-key.pem Config/kubernetes.pem \
Config/service-account-key.pem Config/service-account.pem root@${instance}:~/
done
```
